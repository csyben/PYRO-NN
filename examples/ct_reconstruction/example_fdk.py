...
class nn_model:
    def __init__(self, geometry):
        ...
        self.cosine_weight = tf.compat.v1.get_variable(name='cosine_weight', dtype=tf.float32,
                                             initializer=ct_weights.cosine_weights_3d(self.geometry), trainable=False)

        self.redundancy_weight = tf.compat.v1.get_variable(name='redundancy_weight', dtype=tf.float32,
                                             initializer=ct_weights.parker_weights_3d(self.geometry), trainable=False)

        self.filter = tf.compat.v1.get_variable(name='reco_filter', dtype=tf.float32, initializer=ram_lak_3D(self.geometry), trainable=False)
    ...
    def model(self, sinogram):
        ...
        self.weighted_sino_fft = tf.compat.v1.fft(tf.cast(self.redundancy_weighted_sino, dtype=tf.complex64))
        self.filtered_sinogram_fft = tf.multiply(self.weighted_sino_fft, tf.cast(self.filter,dtype=tf.complex64))
        self.filtered_sinogram = tf.compat.v1.real(tf.compat.v1.ifft(self.filtered_sinogram_fft))
